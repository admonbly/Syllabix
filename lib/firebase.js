...
// Pose les cookies de session via l'API serveur (HttpOnly, signés).
// Pas de fallback client : un cookie posé en JavaScript serait forgeable.
/**
 * Pose les cookies de session et renvoie la destination choisie par le serveur
 * selon le rôle ('/org' pour un administrateur d'organisation, sinon '/dashboard').
 * Renvoie null si la session n'a pas pu être établie — l'appelant décide alors.
 */
async function setSessionCookies(token) {
  try {
    ...
    if (!res.ok) {
      ...
      console.error('setSessionCookies: API session en erreur', res.status, data.error ?? '');
      return null;
    }
    const data = await res.json().catch(() => ({}));
    return data.landing ?? null;
  } catch (err) {
    console.error('setSessionCookies:', err);
    return null;
  }
}
...
async function clearSessionCookies() {
  try {
    ...
    console.error('clearSessionCookies:', err);
  }
}

/**
 * Initialise le profil Firestore côté serveur (Admin SDK) après la LIAISON d'une
 * session anonyme. Nécessaire car le client ne peut pas poser email/role/createdAt
 * sur un document déjà créé (règles), et le merge serveur préserve les badges
 * décrochés pendant le défi. Voir /api/auth/init-profile.
 */
async function initServerProfile(token, extra = {}) {
  try {
    ...
    console.error('initServerProfile:', err);
  }
}
...
export const authFunctions = {
  // Ouvre une session ANONYME si aucune session n'existe (défi public).
  // Idempotent : renvoie l'utilisateur courant s'il y en a déjà un.
  ...
  signUp: async (email, password, { firstName, lastName, role = 'LEARNER', dateOfBirth = null, parentalConsentGiven = false, phoneNumber = '' } = {}) => {
    try {
      ...
      const linkingAnon = current && current.isAnonymous;

      let user;
      if (linkingAnon) {
        // LIAISON : on conserve l'uid anonyme (donc les badges du défi) en le
        // transformant en compte email/mot de passe permanent.
      ...
      if (linkingAnon) {
        // Le doc peut déjà exister (badges décrochés en anonyme) : on passe par le
        // serveur (Admin SDK, merge) pour poser le profil sans écraser les badges.
        await initServerProfile(token, {
          firstName: firstName ?? '', lastName: lastName ?? '', phoneNumber: phoneNumber ?? '',
          dateOfBirth, parentalConsentGiven,
        });
      } else {
        const userData = {
          email: user.email,
  ...
  signInWithGoogle: async () => {
    ...
    // LIAISON d'une session anonyme (défi) → conserve l'uid et les badges.
    const result = linkingAnon
    ...
    const token = await user.getIdToken(true);
    const landing = await setSessionCookies(token);

    // Cas liaison : profil posé côté serveur (merge → badges préservés),
    // complétion requise (pas de date de naissance).
    if (linkingAnon) {
      await initServerProfile(token, {});
      return { user, profileComplete: false, landing };
    }
    ...
    return { user, profileComplete, landing };
  },
};
...
export const userDB = {
  ...
  // NOTE : la progression, les certificats et les badges sont écrits
  // EXCLUSIVEMENT côté serveur (/api/exam/submit, Admin SDK).
  // Aucune fonction d'écriture ne doit exister ici pour ces collections.
