...
// Pose les cookies de session via l'API serveur (HttpOnly) avec fallback client
async function setSessionCookies(token) {
  try {
    ...
    // Fallback client-side si l'API est indisponible
    const maxAge = 6 * 60 * 60;
    const opts = `path=/; SameSite=Strict; Secure; Max-Age=${maxAge}`;
    document.cookie = `syllabix_session=1; ${opts}`;
    document.cookie = `syllabix_last_activity=${Date.now()}; ${opts}`;
  }
}
...
async function clearSessionCookies() {
  try {
    ...
    document.cookie = 'syllabix_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Strict; Secure';
    document.cookie = 'syllabix_last_activity=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Strict; Secure';
  }
}
