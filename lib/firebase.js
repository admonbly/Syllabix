...
export const authFunctions = {
  ...
  signInWithGoogle: async () => {
    ...
    // Cookie en premier — la redirection ne dépend pas de Firestore
    document.cookie = 'syllabix_session=1; path=/; SameSite=Strict';

    // Créer le profil Firestore si première connexion — non bloquant
    try {
      const userRef = doc(db, 'users', user.uid);
      const userSnap = await getDoc(userRef);
      if (!userSnap.exists()) {
      ...
      // Firestore échoue (règles pas encore configurées) → on redirige quand même
      console.warn('Firestore profile creation skipped:', firestoreError.message);
    }
