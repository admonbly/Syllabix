...
export const EXAM_CONFIG = {
  TRAINING: {
    QUESTIONS_COUNT: 5,
    DURATION: null, // Pas de timer en mode entraînement
    SHOW_FEEDBACK: true,
    SAVE_RESULTS: false,
...
/**
 * Récupère X questions aléatoires d'un module (depuis Firestore via quizService)
...
/**
 * Récupère X questions aléatoires de TOUS les modules mélangés (depuis Firestore)
...
/**
 * Récupère X questions d'un module en répétant si nécessaire pour atteindre count
...
export const getModuleQuestionsWithRepeat = async (moduleId, count = 35) => {
  const module = await getModuleById(parseInt(moduleId));
  if (!module) return [];
  const source = module.questions.map((q) => ({
    ...q,
    module: module.module,
    moduleId: module.id,
  }));
